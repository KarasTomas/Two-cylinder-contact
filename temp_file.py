...
# 1. Geometry to tune (must match a file in 'processed_data/')
GEOMETRY_SUFFIX = "gear_179"
...
PLATEAU_THRESHOLD = 0.98  # Linear peak - Defines the vertical threshold (98% of max) for finding the peak plateau.
FLAT_SLOPE_TOLERANCE = 0.5  # Linear slope - Defines a "flat" slope for plateau detection (relative to max slope).

# Point allocation ratios (should sum roughly to 1.0)
PLATEAU_POINTS_RATIO = 0.15  # 15% of points on the peak plateau.
MAIN_SLOPE_POINTS_RATIO = 0.50  # 50% of points on the steepest up/down slopes.
SHALLOW_SLOPE_POINTS_RATIO = 0.35  # 35% of points on the shallow regions.
